...
function changePage(page) {
    ...
    // Atualiza a tabela conforme a nova página
    ...
    buttons.forEach(button => {
        if (!button.classList.contains('seta')) {
            button.classList.remove('active');
        }
    });

    buttons.forEach(button => {
        if (!button.classList.contains('seta') && button.textContent.trim() == page) {
            button.classList.add('active');
        }
    });

    // Reatribui os event listeners aos botões
...
document.addEventListener('DOMContentLoaded', (event) => {
    changePage(currentPage); // Inicializa com a primeira página
});

// Função para reatribuir os event listeners aos botões dos modais
